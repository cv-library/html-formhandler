...
sub wrap_field {
    ...
    my $form_actions =
        ( $self->name eq 'form_actions' ||
            $self->type_attr eq 'submit' ||
    ...
    my $div_class =
        $self->type eq 'Hidden' ? undef : $form_actions ? "form-actions" : "control-group";
    ...
    unless ( $self->get_tag('no_errors') ) {
        $output .= qq{\n<span class="help-inline">$_</span>} for $result->all_errors;
...
sub do_prepend_append {
    ...
    if ( my $iab_tag = $self->get_tag('input_append_button') ) {
        ...
        foreach my $btn (@buttons) {
            $rendered_widget =
                qq{$rendered_widget<button type="button" class="btn">$btn</button>};
        }
        push @class, 'input-append';
    }
